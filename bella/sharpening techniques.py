...
orignal_image = cv2.imread(r"C:\Users\bella\Downloads\JBG040 Data Challenge 1\pneumothorax.png")

# unsharp masking
unsharp_mask_1 = unsharp_mask(orignal_image, radius=1, amount=1)
unsharp_mask_2 = unsharp_mask(orignal_image, radius=1, amount=2)
unsharp_mask_3 = unsharp_mask(orignal_image, radius=5, amount=1)
unsharp_mask_4 = unsharp_mask(orignal_image, radius=5, amount=2)
unsharp_mask_5 = unsharp_mask(orignal_image, radius=20, amount=1)
unsharp_mask_6 = unsharp_mask(orignal_image, radius=20, amount=2)
...
ax[0].imshow(orignal_image, cmap=plt.cm.gray)
...
CLAHE_1 = equalize_adapthist(orignal_image, kernel_size=None, clip_limit=0.01, nbins=256)
CLAHE_2 = equalize_adapthist(orignal_image, kernel_size=None, clip_limit=0.02, nbins=256)
CLAHE_3 = equalize_adapthist(orignal_image, kernel_size=None, clip_limit=0.03, nbins=256)
CLAHE_4 = equalize_adapthist(orignal_image, kernel_size=None, clip_limit=0.04, nbins=256)
CLAHE_5 = equalize_adapthist(orignal_image, kernel_size=None, clip_limit=0.5, nbins=256)
...
# side-by-side comparison of unsharp masking, unsharp masking + histogram equalization with CLAHE
...
ax[1].imshow(unsharp_mask_4, cmap=plt.cm.gray)
ax[1].set_title('Unsharp masking')
ax[2].imshow(equalized_image_4, cmap=plt.cm.gray)
ax[2].set_title('Unsharp masking and histogram equalization')
ax[3].imshow(CLAHE_5, cmap=plt.cm.gray)
...
data: torch.tensor = kornia.utils.image_to_tensor(orignal_image, keepdim=False)
data = data.float() / 255

sharpen = kornia.filters.UnsharpMask((25,25), (15,15))
sharpened_tensor = sharpen(data)
sharpened_tensor = torch.clamp(sharpened_tensor, min=0.0, max=1.0)
image_eq = kornia.enhance.equalize(sharpened_tensor)
sharpened_image = kornia.utils.tensor_to_image(image_eq) 
...
PIL_original = Image.open(r"C:\Users\bella\Downloads\JBG040 Data Challenge 1\pneumothorax.PIL")
transform = T.Compose([T.PILToTensor()])
img_tensor = transform(PIL_original)
img_tensor_org = TF.to_pil_image(img_tensor, mode=None)

img_sharp = TF.adjust_sharpness(img_tensor, sharpness_factor=5)
img_bright = TF.to_pil_image(img_sharp, mode=None)
